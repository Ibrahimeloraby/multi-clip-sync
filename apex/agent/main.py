...
class ApexAgent:
    ...
    def __init__(self) -> None:
        ...
        self._running = False
        self._last_sentiment_refresh = 0.0  # epoch seconds
    ...
    async def _run_once(self) -> AgentState:
        ...
        # 4. Sentiment — refresh every 5 minutes to avoid API overuse
        ...
        for sym, sig in signals.items():
            ...
            existing_qty = pos_map.get(sym, 0.0)
            risk_result = self._risk.evaluate(sig, account, existing_qty)

            if risk_result.approved:
                await self._orders.place_order(sig, risk_result.quantity)
            else:
